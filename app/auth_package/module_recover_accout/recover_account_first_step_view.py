...
class AccountRecoverFirstStepView(View):
    methods = ['GET', 'POST']
    ...
    def dispatch_request(self):
        ...
        if request.method == 'POST':
            ...
            if validate_form_fields(request.form):
                ...
                try:
                    status, user = self.userModel.get_user_by_email(username)
                        ...
                        if status and token is not None:
                            ...
                            if status and two_fa is not None: 
                                if user.is_active() == True:
                                    ...
                                    return redirect(url_for('auth.recover.user_account_st_2',user_token=token.token))     
                            else:
        ...
        return render_template(self.template, title='Account recovery')
