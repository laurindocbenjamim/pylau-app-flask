    ...
    def dispatch_request(self, user_token) -> any:
        ...
        if request.method == "POST":
            ...
            if comment is None or comment == "":
                ...
                status = 400
                message = (
                    "Enter a valid comment with with no special characteres to analyse"
                )
                category = "error"
            else:
                status = 200
            ...
            return jsonify(
                {"message": message, "category": category, "emotions": [emotions]},
                status,
            )

        return render_template(
            self._template, title="Emotion Detector", user_token=user_token
        )
